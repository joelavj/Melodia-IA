...
class QueueService:
    ...
    def __init__(self) -> None:
        self._id_queue = playlist_repository.queue_id()
        self._queue = Queue()
        self.reload()

    # Recharge les données
    def reload(self):
        self._queue.queue = playlist_repository.get_songs(self._id_queue)
    ...
    def add(self, id_song: int):
        if not playlist_repository.song_is_here(self._id_queue, id_song):
            playlist_repository.save(self._id_queue, id_song)
            self.reload()

    # Supprimer un morceau
    def remove(self, id_song: int):
        playlist_repository.clear_playlist(self._id_queue)
        self._queue.queue = [song for song in self._queue.queue if song.id != id_song]
        for song in self._queue.queue:
            playlist_repository.save(self._id_queue, song.id)
        self.reload()
    ...
    # La liste de morceaux dans la file d'attente
    def songs(self):
    ...
    def contains(self, song: Song) -> bool:
        return playlist_repository.song_is_here(self._id_queue, song.id)
