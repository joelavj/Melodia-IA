    ...
    def afficher_albums(self, albums=None):
        try:
            ...
            if not albums:
                ...
                return
            # AlbumCard a une taille fixe (160x230) : donner weight=1 aux
            # colonnes les étirait avec la largeur de la fenêtre, chaque
            # carte restant centrée dans sa cellule -> écarts entre cartes
            # irréguliers et croissants. weight=0 + sticky="nw" donne une
            # grille compacte et régulière, alignée en haut à gauche.
            for c in range(self.GRID_COLUMNS):
                self.frame_content.grid_columnconfigure(c, weight=0)
            for i, album in enumerate(albums):
                ...
                card.grid(row=i // self.GRID_COLUMNS, column=i % self.GRID_COLUMNS, padx=8, pady=8, sticky="nw")
